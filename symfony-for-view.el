...
(defun symfony-view:create-partial-from-selection ()
  ...
  (if mark-active
      (save-excursion
        (let ((name (read-string "Partial name (without _ and extension)? "))
          ...
          (insert (concat "<?php echo render :partial => \"" name "\" ?>"))
...
(defun symfony-view:create-helper-from-block (&optional helper-name)
...
(defun symfony-view:switch-to-action ()
...
(defun symfony-view:switch-with-menu ()
  ...
  (let ((menu (symfony-core:menu-of-views (symfony-core:current-controller) t))
    ...
    (add-to-list 'menu (list "Controller" 'symfony-view:switch-to-action))
    (setq item
          (symfony-core:menu
           (list (concat "View "
                         (symfony-core:current-controller)
...
(defun symfony-for-view ()
  "Enable RHTML configurations."
  (interactive)
  (setq symfony-primary-switch-func 'symfony-view:switch-to-action)
  (setq symfony-secondary-switch-func 'symfony-view:switch-with-menu)
  (if (boundp 'mmm-mode-map)
      (progn
        (define-key mmm-mode-map (kbd "\C-c p") 'symfony-view:create-partial-from-selection)
        (define-key mmm-mode-map (kbd "\C-c b") 'symfony-view:create-helper-from-block))
    (progn
      (local-set-key (kbd "\C-c p") 'symfony-view:create-partial-from-selection)
      (local-set-key (kbd "\C-c b") 'symfony-view:create-helper-from-block))))

(provide 'symfony-for-view)
