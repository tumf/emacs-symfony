...
;; $URL: svn+ssh://phpforge/var/svn/emacs-symfony/trunk/symfony-lib.el $
;; $Id: symfony-lib.el 88 2007-01-29 21:21:06Z dimaexe $
...
(defun symfony-lib:run-primary-switch ()
  "Run the primary switch function."
  (interactive)
  (if symfony-primary-switch-func
      (apply symfony-primary-switch-func nil)))

(defun symfony-lib:run-secondary-switch ()
  "Run the secondary switch function."
  (interactive)
  (if symfony-secondary-switch-func
      (apply symfony-secondary-switch-func nil)))

;;;;; Non Symfony realted helper functions ;;;;;
...
(defun symfony-browse-api-url (url)
  "Browse preferentially with Emacs w3m browser."
  (if symfony-browse-api-with-w3m
      (when (fboundp 'w3m-find-file)
        (w3m-find-file (remove-prefix url "file://")))
    (symfony-alternative-browse-url url)))

(defun symfony-alternative-browse-url (url &rest args)
  "Fix a problem with Internet Explorer not being able to load
URLs with anchors via ShellExecute. It will only be invoked it
the user explicit sets `symfony-use-alternative-browse-url'."
  (if (and (eq system-type 'windows-nt) symfony-use-alternative-browse-url)
      (w32-shell-execute "open" "iexplore" url)
    (browse-url url args)))

;; snippets related

;; (setq pp (create-snippets-and-menumap-from-dsl symfony-snippets-menu-list))
...
;; (defadvice php-indent-line (around mmm-sandbox-php-indent-line)
;;   (if (and (fboundp 'mmm-overlay-at)
;;            (mmm-overlay-at (point)))
;;       (mmm-run-indent-with-sandbox 'php-indent-line)
;;     ad-do-it))
;; (ad-activate 'php-indent-line)


;; Cross define functions from my rc files

(defun symfony-lib:indent-or-complete ()
...
(defun symfony-lib:replace-tokens (tokens)
  (mapcar
   #'(lambda (token)
       (goto-char (point-min))
       (while (search-forward
	       (upcase (format "##%s##" (car token))) nil t)
	 (let ((to (cdr token)))
	   (unless (stringp to) (setq to (car to)))
	   (replace-match to t)))) tokens))
